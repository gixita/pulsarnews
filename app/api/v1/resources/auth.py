...
from app.api.v1.exceptions import ValidationException
...
register_model = v1_api.model('Register', {
    'acct': fields.String(required=True),
...
class Register(Resource):
    ...
    def post(self):
        if not re.search(self.USERNAME_REGEXP, v1_api.payload['acct']):
            raise ValidationException(error_field_name='acct',
                                      message='4-16 symbols, can contain A-Z, a-z, 0-9, _ \
                                      (_ can not be at the begin/end and can not go in a row (__))')

        if not re.search(self.PASSWORD_REGEXP, v1_api.payload['pw']):
            raise ValidationException(error_field_name='pw',
                                      message='6-64 symbols, required upper and lower case letters. Can contain !@#$%_')

        if User.query.filter_by(username=v1_api.payload['acct']).first():
            raise ValidationException(error_field_name='acct', message='This username is already exists')

        user = User(username=v1_api.payload['acct'], password=v1_api.payload['pw'])
        db.session.add(user)
        db.session.commit()
        return user
...
class Login(Resource):
    ...
    def post(self):
        ...
        """
        user = User.query.filter_by(username=request.form['acct']).first()
        if not user:
            auth_ns.abort(401, 'Incorrect username or password')

        from werkzeug.security import check_password_hash
        if check_password_hash(user.password_hash, request.form['pw']):
